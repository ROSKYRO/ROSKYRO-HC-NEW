...
## Timestamps (UTC in the database, `Z` on the wire)

Every `DateTime` column stores **naive UTC**. Every UTC datetime the API returns
is serialised as ISO-8601 **with a trailing `Z`** (`2026-09-19T09:20:00.000Z`),
so browsers convert it to the viewer's local time correctly. Without the `Z`,
`new Date("2026-09-19T09:20:00")` is read as *local* time and everything shows
5h30m early in India (this used to affect the admin panel, the hospital
dashboard and the customer's live visit timer).

- Response schemas: type UTC fields as `UTCDateTime` (`app/schemas/utc_types.py`),
  never plain `datetime`. `python test_timezone.py` scans every schema and
  fails if a bare `datetime` field sneaks in.
- A route that returns a raw `dict` (no `response_model`) must format its
  datetimes with `to_iso_z()` (`app/core/timeutil.py`) itself.
- Text ROSKYRO composes itself (e.g. the hospital's WhatsApp discharge notice)
  uses `format_ist()`, since no browser is there to localise it.
- The one intentional exception is `TransportRequest.requested_time`, a
  wall-clock time the member types and that is stored/returned exactly as typed.
- Not changed: day boundaries for billing/"today" (`datetime.utcnow().date()`)
  still roll over at 00:00 UTC = 05:30 IST.
