...
from app.schemas.hospital import (
    ...
    PatientCaseOut, PatientCaseCreateIn, PatientCaseStatusIn, DailyAssignmentOut, AssignOfficerIn,
    ForceCloseDischargeIn, AssignmentStatusIn, OfficerLinkOut, CaseAlertOut, OfficerRosterOut,
    OfficerRosterCaseOut, OfficerPortalLinkOut,
)
...
# The hospital has NO write access here at all: ROSKYRO admin/ops opens
# every case (after being handed a few patient details verbally by the
# hospital — phone/WhatsApp/in person, never through any hospital login) and
# can cancel a mistakenly-opened one. Discharge itself is confirmed solely
# by the assigned officer (see routers/officer.py) or force-closed here.
# ---------------------------------------------------------------------------
...
def _to_case_out(c: PatientCase) -> PatientCaseOut:
    ...
    return PatientCaseOut(
        ...
        can_dispute_discharge=False,  # admin never "disputes" — that's the hospital's action
...
@router.post("/hospitals/{hospital_id}/patients", response_model=PatientCaseOut)
def create_patient_case(
    ...
    """ROSKYRO admin/ops opens a case — short patient/attendant details,
    handed over verbally by the hospital (never typed in by the hospital
    itself; the Hospital Console has no create action). ROSKYRO's assigned
    Relationship Officer takes it from here."""
    hospital = db.query(Hospital).get(hospital_id)
    if not hospital:
        raise HTTPException(status_code=404, detail="Hospital not found")
    if hospital.per_patient_daily_rate is None:
        raise HTTPException(
            status_code=400,
            detail="This hospital's per-patient daily billing rate hasn't been set up yet — configure it before opening cases.",
        )
...
def update_patient_case_status(
    ...
    """Cancel a mistakenly-opened case (before RO coverage started, or
    opened for the wrong hospital/patient). Discharge is NOT set through
    here — see routers/officer.py (officer confirms) or force-close below."""
    if payload.status == PatientCaseStatus.discharged:
        raise HTTPException(
            status_code=400,
            detail="A case can't be marked discharged directly — it needs the assigned "
                   "officer's confirmation (routers/officer.py) or a force-close.",
        )
...
def list_all_patient_cases(
    ...
    if status and status != "all":
        ...
        if status_filter == PatientCaseStatus.active:
            # "Active" also covers the (now effectively unreachable, but
            # still a valid enum value) pending_discharge status, so an old
            # record from before single-sided confirmation still shows up.
...
def _guess_image_mime(b64: str) -> str:
    """Legacy-only: pre-Drive-link cases still have the photo as bare base64
    (no data: prefix) in discharge_photo_base64. Sniff the first bytes so the
    browser gets the right MIME type; JPEG is what the old compress-before-
    upload step produced, so it is the default."""
...
@router.get("/patients/{case_id}/discharge-photo")
def get_discharge_photo(
    case_id: int,
    db: Session = Depends(get_db),
    admin: User = Depends(require_admin),
):
    """ROSKYRO Admin only: the officer's discharge-proof photo + capture
    time/GPS. Current cases store just a Google Drive share link
    (discharge_photo_url) — the photo itself is shared on WhatsApp and saved
    to Drive manually, never uploaded through the app, which is what keeps
    this out of PatientCaseOut/Postgres entirely now. Older cases confirmed
    before this change may still only have the legacy base64 blob, which is
    served here for as long as it's still on file."""
    case = db.query(PatientCase).filter(PatientCase.id == case_id).first()
    if not case:
        raise HTTPException(status_code=404, detail="Patient case not found")
    if not case.discharge_photo_url and not case.discharge_photo_base64:
        raise HTTPException(status_code=404, detail="No discharge photo is on file for this case.")

    result = {
        # This route returns a plain dict (no response_model), so the UTC "Z"
        # formatting the schemas apply automatically has to be done by hand.
        "taken_at": to_iso_z(case.discharge_photo_at) if case.discharge_photo_at else None,
        "lat": case.discharge_lat,
        "lng": case.discharge_lng,
        "discharged_at": to_iso_z(case.discharged_at) if case.discharged_at else None,
        "patient_name": case.patient_name,
    }
    if case.discharge_photo_url:
        result["photo_url"] = case.discharge_photo_url
        result["is_drive_link"] = True
    else:
        result["data_url"] = f"data:{_guess_image_mime(case.discharge_photo_base64)};base64,{case.discharge_photo_base64}"
        result["is_drive_link"] = False
    return result
...
def _to_invoice_out(inv: HospitalInvoice) -> HospitalInvoiceOut:
    ...
    return HospitalInvoiceOut(
        ...
        reported_payment_method=inv.reported_payment_method,
        reported_payment_reference=inv.reported_payment_reference,
        reported_payment_note=inv.reported_payment_note,
        reported_at=inv.reported_at,
        cases=cases_out,
    )
...
def list_all_invoices(
    ...
    parsed_status = parse_enum_filter(HospitalInvoiceStatus, status, "status")
    if parsed_status:
        query = query.filter(HospitalInvoice.status == parsed_status)
...
def mark_paid(
    ...
    """Admin confirms a hospital's payment has actually come in — either
    verified against what the hospital self-reported (payment_reported,
    via the Hospital Console's report-payment action) or straight from a
    bank/cheque/UPI check with no prior report. When the admin doesn't type
    a fresh reference/note, whatever the hospital reported is carried over,
    so a plain confirm doesn't lose that trail."""
    invoice = (
        db.query(HospitalInvoice)
        .options(joinedload(HospitalInvoice.cases), joinedload(HospitalInvoice.hospital))
    ...
    reference = payload.payment_reference or invoice.reported_payment_reference
    note = payload.payment_note or invoice.reported_payment_note
    try:
        mark_invoice_paid(invoice, admin.id, reference, note)
    except InvoiceError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    db.commit()
    db.refresh(invoice)
