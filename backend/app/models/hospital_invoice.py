...
class HospitalInvoiceStatus(str, enum.Enum):
    unpaid = "unpaid"
    # Hospital has self-reported a payment (cheque/netbanking/UPI/cash) via
    # its own read-only console — ROSKYRO admin still verifies and flips this
    # to paid; this state exists so ROSKYRO knows a claim is waiting on them
    # instead of the hospital having to call/WhatsApp to say "we paid".
    payment_reported = "payment_reported"
    paid = "paid"


class HospitalPaymentMethod(str, enum.Enum):
    cheque = "cheque"
...
class HospitalInvoice(Base):
    ...
    # (before ROSKYRO admin verifies and marks it paid via the fields above).
    reported_payment_method = Column(Enum(HospitalPaymentMethod), nullable=True)
    reported_payment_reference = Column(String, nullable=True)
    reported_payment_note = Column(Text, nullable=True)
    reported_at = Column(DateTime, nullable=True)
    reported_by_id = Column(Integer, ForeignKey("users.id"), nullable=True)  # hospital staff who reported it
