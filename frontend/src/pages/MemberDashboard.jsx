...
const TABS = [
  ...
  { id: "Doctor Network", label: "Doctor Network", icon: Stethoscope },
...
export default function MemberDashboard() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 md:py-12">
      ...
      {tab === "Doctor Network" && <DoctorNetworkTab />}
...
function DoctorNetworkTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-3xl border border-ink/10 p-6 shadow-xs">
        <div className="text-xs font-bold uppercase tracking-wider text-ink/50 flex items-center gap-1.5 mb-3">
          <Stethoscope className="w-3.5 h-3.5 text-violet" />
          Priority Access to doctors &amp; hospitals — included with your membership
        </div>
        <form onSubmit={search} className="grid sm:grid-cols-3 gap-3">
          <input
          ...
          <input
            ...
            className="rounded-xl border border-ink/15 px-3.5 py-2.5 text-xs outline-none focus:ring-2 focus:ring-violet"
          />
          <button className="rounded-full bg-brand-gradient text-white text-xs font-bold py-2.5">
      ...
      <div className="space-y-3">
        {(results || []).slice(0, 6).map((p) => (
          <Link
            key={p.id}
            to={`/membership/priority-access/${p.id}`}
            className="block bg-white border border-ink/10 rounded-2xl p-4 shadow-xs hover:border-violet/40 transition-colors"
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-violet/10 text-violet whitespace-nowrap">
                {p.priority_access_status === "available" ? "Priority: Available" : "Priority: By request"}
              </span>
            </div>
          </Link>
        ))}
        {results && results.length === 0 && (
          <div className="p-8 text-center bg-slate-50 rounded-2xl border border-ink/10 text-xs text-ink/50">
            No partners found for that search — try a different city or specialty.
          </div>
        )}
      </div>
      ...
      <Link
        ...
        className="inline-flex items-center gap-2 text-xs font-bold text-violet hover:underline"
      >
        Open the full Priority Access directory →
      </Link>
    </div>
  );
}
