...
function PortalCaseCard({ c, muted }) {
  return (
    <div className={`bg-white rounded-2xl border p-4 space-y-2 text-sm ${muted ? "border-ink/10 opacity-60" : "border-ink/10"}`}>
      ...
      {c.status === "active" && c.discharge_link_token && (
        <div className="text-xs text-ink/60 bg-slate-50 border border-ink/10 rounded-lg px-3 py-2">
          <a
            href={`/officer/discharge/${c.discharge_link_token}`}
            className="block font-bold text-violet hover:underline"
          >
