  ...
  return (
    <div className="max-w-6xl mx-auto px-5 py-12">
      ...
      <p className="text-ink/60 mb-8">
        ...
        day by day until that officer confirms the actual discharge date & time (with a photo, as proof). The
        hospital has no operational role: open every case here, on their behalf, after they hand over a few
        patient details verbally — they only ever view their own cases/invoices and pay.
      </p>
      ...
      {tab === "hospitals" && (
        <HospitalsTab hospitals={hospitals} onChanged={loadHospitals} onCaseCreated={() => setTab("board")} />
      )}
    </div>
  );
}
...
function CaseRow({ c, agents, onAssigned }) {
  ...
  const [photo, setPhoto] = useState(null);        // { data_url, taken_at, lat, lng } once fetched
  const [showPhoto, setShowPhoto] = useState(false);
  const [photoLoading, setPhotoLoading] = useState(false);
  const [photoError, setPhotoError] = useState("");

  // Officer undid and re-confirmed with a new photo -> drop the cached old one.
  useEffect(() => {
    setPhoto(null);
    setShowPhoto(false);
    setPhotoError("");
  }, [c.discharge_photo_at]);

  // The photo bytes are not part of the case list (too heavy) — fetch them on
  // demand, once, when the admin actually wants to look at the proof.
  async function togglePhoto() {
    ...
    try {
      const { data } = await api.get(`/admin/hospital-program/patients/${c.id}/discharge-photo`);
      setPhoto(data);
    } catch (err) {
      setPhotoError(err.response?.data?.detail || "Could not load the discharge photo.");
    } finally {
  ...
  // Force-close is for a case the officer never confirms — which, since
  // confirming now closes the case immediately (no more pending_discharge
  // limbo), just means: still active/pending_discharge, i.e. not yet discharged/cancelled.
  const canForceClose = c.status === "active" || c.status === "pending_discharge";
    ...
    <div className="border border-ink/10 rounded-card p-5">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {c.status === "discharged" && c.hospital_disputed_at && (
            <span className="text-xs font-semibold px-3 py-1 rounded-full bg-rose-100 text-rose-700">
              Hospital disputed this discharge
            </span>
          )}
          {!c.officer_confirmation_required && c.status !== "discharged" && (
            <span className="text-[10px] text-ink/40">No RO assigned — force-close will be needed to close this</span>
          )}
        </div>
      </div>
      ...
      <div className="text-xs text-ink/40 mb-3">
        Officer confirmed: {c.officer_discharge_at ? `${new Date(c.officer_discharge_at).toLocaleString()}${c.has_discharge_photo ? " · photo on file" : ""}` : "no"}
        {c.hospital_notified_at && ` · hospital notified ${new Date(c.hospital_notified_at).toLocaleString()}`}
        {c.hospital_disputed_at && (
          <span className="block text-rose-600 font-semibold mt-1">
            Hospital disputed this on {new Date(c.hospital_disputed_at).toLocaleString()}: "{c.hospital_dispute_note}"
          </span>
        )}
      ...
      {c.has_discharge_photo && (
        <div className="mb-3">
          ...
          >
            {showPhoto ? "Hide discharge photo" : "View discharge photo"}
          </button>
          {showPhoto && (
            <div className="mt-2 rounded-xl border border-ink/10 bg-ink/[0.02] p-3 max-w-md">
              {photoLoading && <p className="text-xs text-ink/50">Loading photo…</p>}
              {photoError && <p className="text-xs text-rose-600">{photoError}</p>}
              {photo && photo.is_drive_link && (
                <>
                  <a
                    href={photo.photo_url}
                    target="_blank" rel="noreferrer"
                    className="inline-flex items-center gap-1.5 text-sm font-semibold text-violet underline underline-offset-2"
                  >
                    Open discharge photo on Google Drive ↗
                  </a>
                  <p className="text-[11px] text-ink/50 mt-2">
                    Taken {photo.taken_at ? new Date(photo.taken_at).toLocaleString() : "—"}
                    ...
                    {" · saved on ROSKYRO's Google Drive"}
                  </p>
                </>
              )}
              {photo && !photo.is_drive_link && (
                <>
                  <a href={photo.data_url} target="_blank" rel="noreferrer">
                    <img
                      src={photo.data_url}
                      alt={`Discharge proof — ${c.patient_name}`}
                  ...
                  <p className="text-[11px] text-ink/50 mt-2">
                    ...
                    {" · legacy photo stored in the database · tap to open full size"}
                  </p>
                </>
              )}
            </div>
      ...
      <p className="text-[10px] text-ink/40 mt-2">
        ...
        today's officer even after the To date passes) until they confirm the actual discharge (with a photo).
        Swapping the officer here revokes the previous officer's link and clears any confirmation they'd given.
      </p>
      ...
      {showForceClose && (
        <form onSubmit={forceClose} className="mt-3 border-t border-ink/10 pt-3 space-y-2">
          <p className="text-xs text-ink/60">
            Use this only when the assigned officer has stopped responding (lost their phone, left the job). It
            fills in their confirmation, closes the case so billing stops, revokes their link, and permanently
            records that this was an override rather than a clean officer confirmation.
          </p>
...
// Today's date in the admin's own timezone (YYYY-MM-DD). `toISOString()` would
// give the UTC date, which is still "yesterday" for the first 5h30m of an IST day.
...
function HospitalsTab({ hospitals, onChanged, onCaseCreated }) {
  const [showNewHospital, setShowNewHospital] = useState(false);
  const [showStaffFor, setShowStaffFor] = useState(null);
  const [showNewPatientFor, setShowNewPatientFor] = useState(null);
        ...
        {hospitals.map((h) => (
          <div key={h.id} className="border border-ink/10 rounded-card p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowNewPatientFor(showNewPatientFor === h.id ? null : h.id)} className="text-xs font-semibold text-violet">
                  {showNewPatientFor === h.id ? "Cancel" : "+ New patient"}
                </button>
            ...
            {showNewPatientFor === h.id && (
              <NewPatientCaseForm
                hospitalId={h.id}
                onCreated={() => { setShowNewPatientFor(null); onCaseCreated(); }}
              />
            )}
...
function NewPatientCaseForm({ hospitalId, onCreated }) {
  ...
  async function submit(e) {
    ...
    setSaving(true);
    try {
      await api.post(`/admin/hospital-program/hospitals/${hospitalId}/patients`, {
  ...
  return (
    <form onSubmit={submit} className="mt-4 border-t border-ink/10 pt-4 space-y-3 max-w-lg">
      <p className="text-xs text-ink/50">
        Whatever the hospital has handed over verbally — just the basics. You'll assign a Relationship Officer next,
        from the Ops Board.
      </p>
      <div className="grid grid-cols-2 gap-2">
        <input required placeholder="Patient name" value={form.patient_name} onChange={(e) => set("patient_name", e.target.value)} className="text-sm border border-ink/15 rounded-lg px-3 py-2" />
        <input type="number" min="0" placeholder="Age (optional)" value={form.patient_age} onChange={(e) => set("patient_age", e.target.value)} className="text-sm border border-ink/15 rounded-lg px-3 py-2" />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <input placeholder="Attendant name (optional)" value={form.attendant_name} onChange={(e) => set("attendant_name", e.target.value)} className="text-sm border border-ink/15 rounded-lg px-3 py-2" />
        <input required type="tel" placeholder="Attendant phone" value={form.attendant_phone} onChange={(e) => set("attendant_phone", e.target.value)} className="text-sm border border-ink/15 rounded-lg px-3 py-2" />
      </div>
      <input placeholder="Ward / room (optional)" value={form.ward_or_room} onChange={(e) => set("ward_or_room", e.target.value)} className="w-full text-sm border border-ink/15 rounded-lg px-3 py-2" />
      <textarea
        placeholder="Short note (optional) — e.g. cardiac, post-op, needs help with mobility"
        rows={2} value={form.short_note} onChange={(e) => set("short_note", e.target.value)}
      ...
      <button disabled={saving} className="text-sm font-semibold px-4 py-2 rounded-full bg-violet text-white disabled:opacity-60">
        {saving ? "Saving…" : "Open patient case"}
      </button>
    </form>
  );
}
...
function InvoiceCard({ inv, onChanged }) {
  ...
  function openPayForm() {
    setReference(inv.reported_payment_reference || "");
    setNote(inv.reported_payment_note || "");
    setShowPay(true);
  }
  ...
  return (
    <div className="border border-ink/10 rounded-card p-5">
        ...
        <div className="text-right">
          ...
          <span
            className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${
              ...
              : inv.status === "payment_reported" ? "bg-blue-50 text-blue-700"
              : "bg-amber-50 text-amber-700"
            }`}
          >
            {inv.status === "payment_reported" ? "payment reported" : inv.status}
          </span>
        </div>
      </div>

      {inv.status === "payment_reported" && (
        <div className="text-xs text-blue-700 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2 mt-2">
          Hospital reported paying by <strong className="capitalize">{inv.reported_payment_method}</strong>
          {inv.reported_at ? ` on ${new Date(inv.reported_at).toLocaleDateString()}` : ""}
          {inv.reported_payment_reference ? ` · ref: ${inv.reported_payment_reference}` : ""}
          {inv.reported_payment_note ? ` · "${inv.reported_payment_note}"` : ""} — verify and confirm below.
        </div>
      )}
      ...
      <div className="flex gap-4 mt-3 items-center">
        ...
        {inv.status !== "paid" && (
          <button type="button" onClick={() => (showPay ? setShowPay(false) : openPayForm())} className="text-xs font-semibold text-violet ml-auto">
