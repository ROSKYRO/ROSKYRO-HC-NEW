  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-ink/8 shadow-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-18 flex items-center justify-between gap-3">
        ...
        {/* Desktop Navigation Links — 3 Pillars */}
        <nav className="hidden lg:flex items-center gap-1.5 bg-slate-50/80 p-1 rounded-full border border-ink/8">
          {NAV_LINKS.map((l) => {
            ...
            return (
              <Link
                key={l.label}
                to={l.href}
                className={`text-xs font-semibold px-4 py-2 rounded-full transition-all flex items-center gap-1.5 whitespace-nowrap ${
                  isActive
                    ? "bg-violet text-white shadow-xs"
                    : "text-ink/75 hover:text-violet hover:bg-white/80"
                }`}
              >
                <Icon className={`w-3.5 h-3.5 ${isActive ? "text-white" : "text-violet"}`} />
                <span>{l.label}</span>
              </Link>
            );
          })}
                ...
                <Link
                  ...
                  className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                    isActive
                      ...
                      : "text-ink hover:bg-violet/5 hover:text-violet"
                  }`}
                >
                  <span className="flex items-center gap-2.5">
                    <Icon className={`w-4 h-4 ${isActive ? "text-white" : "text-violet"}`} />
                    <span>{l.label}</span>
                  </span>
                  <ChevronRight className={`w-4 h-4 ${isActive ? "text-white/70" : "text-ink/30"}`} />
                </Link>
              );
            })}
          </div>
