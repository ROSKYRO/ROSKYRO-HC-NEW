...
import { SAFETY_WA_LINK, SUPPORT_PHONE_DISPLAY, SUPPORT_EMAIL } from "../../config";
  ...
  return (
    <section id="complaint" className="max-w-3xl mx-auto px-5 py-20">
      ...
      <div className="bg-clay/10 border border-clay/30 rounded-card p-4 text-sm text-clay mb-8">
        🚨 Safety concern? Don't wait — message us now on WhatsApp at{" "}
        <a href={SAFETY_WA_LINK} target="_blank" rel="noreferrer" className="font-semibold underline">{SUPPORT_PHONE_DISPLAY}</a>. It routes straight to our leadership.
      </div>
      ...
      <form onSubmit={submit} className="grid sm:grid-cols-2 gap-4">
        ...
        <input required placeholder="Phone number *" value={form.phone} onChange={update("phone")}
          className="border border-ink/15 rounded-lg px-4 py-2.5 text-sm sm:col-span-1" />
        <input placeholder="Reference ID (if any)" value={form.booking_code} onChange={update("booking_code")}
      ...
      {status === "sent" && <p className="text-sm text-violet mt-4">Received — our team will reach out shortly. <img src="/brand/logo-sm.png" alt="ROSKYRO" className="inline-block w-4 h-4 align-[-3px]" /></p>}
