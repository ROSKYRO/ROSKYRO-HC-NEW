...
/**
 * Priority Access search widget — verified doctors & hospitals directory.
 * Shared between the standalone /membership/priority-access page and the
 * Doctor + Healthcare Concierge plan page, where it's featured at the top
 * since Priority Access is one of that plan's core benefits.
 */
export default function PriorityAccessFinder({ showBreadcrumb = true }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-5 py-16">
      {showBreadcrumb && (
        <Link to="/membership/info" className="text-xs font-semibold text-ink/40 hover:text-violet">
          ROSKYRO Concierge Membership
        </Link>
      )}
      <span className="text-xs font-semibold tracking-wide text-magenta block mt-2">Priority Access Network — a membership benefit</span>
      ...
      {results !== null && (
        <div className="space-y-4 mb-12">
          {results.map((p) => (
            <Link key={p.id} to={`/membership/priority-access/${p.id}`} className="block border border-ink/10 rounded-card p-5 hover:border-violet/40 transition-colors">
      ...
      <div className="bg-mist rounded-card p-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <Link to="/membership/priority-access/apply" className="px-5 py-2.5 rounded-full bg-ink text-parchment font-semibold whitespace-nowrap">
          Become a Partner
        </Link>
      </div>
    </div>
